...
powershell -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command "$r=$env:FAMILIAR_ROOT.TrimEnd('\'); $t=Join-Path $r 'START.bat'; if(Test-Path -LiteralPath $t){$w=New-Object -ComObject WScript.Shell; $s=$w.CreateShortcut((Join-Path $r 'Agent.lnk')); $s.TargetPath=$t; $s.WorkingDirectory=$r; $s.WindowStyle=7; $s.Description='Familiar'; $i=Join-Path $r 'assets\agent.ico'; if(Test-Path -LiteralPath $i){$s.IconLocation=($i+',0')}; $s.Save()}" >nul 2>&1
